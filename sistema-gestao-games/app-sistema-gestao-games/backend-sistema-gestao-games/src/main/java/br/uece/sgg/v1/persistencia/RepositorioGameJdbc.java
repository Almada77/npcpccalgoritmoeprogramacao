...
public class RepositorioGameJdbc implements RepositorioGame {
	...
	private Game mapearResultadoParaGame(ResultSet resultado) throws SQLException {
		...
		String valor6 = resultado.getString("valor6");
		return new Game(id, valor1, valor2, valor3, valor4, valor5, valor6);
	}

	@Override
	public Game salvarGame(Game game) {
		String sql = "INSERT INTO game (valor1, valor2, valor3, valor4, valor5, valor6) VALUES (?, ?, ?, ?, ?, ?)";
		try (PreparedStatement stmt = conexao.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
			stmt.setString(1, game.getValor1());
			stmt.setString(2, game.getValor2());
			stmt.setString(3, game.getValor3());
			stmt.setString(4, game.getValor4());
			stmt.setString(5, game.getValor5());
			stmt.setString(6, game.getValor6()== null ? null : game.getValor6().toString());
			...
			throw new PersistenciaException("Erro ao salvar game.", e);
		}
		return game;
	}
	...
	public void atualizarGame(Game game) {
		String sql = "UPDATE game SET valor1 = ?, valor2 = ?, valor3 = ?, valor4 = ?, valor5 = ?, valor6 = ? WHERE id = ?";
		try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
			stmt.setString(1, game.getValor1());
			stmt.setString(2, game.getValor2());
			stmt.setString(3, game.getValor3());
	...
	public void excluirGame(Game game) {
		...
		try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
			...
			throw new PersistenciaException("Erro ao excluir game.", e);
		}
	}
